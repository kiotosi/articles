...
Владение, референсы и слайсы - одни из сложных тем в Rust, так что не стесняйтесь спрашивать или читать документацию Rust, если чего-то не понимаете.
...
# Референсы
**Референсы** - безопасный указатель на переменную, которая содержит значение. Референсы нужны для того чтобы не перемещать значение переменной внутрь другого контекста.
...
- Референс ссылается *на переменную*, которая является владельцем, а не на сами данные;
- Если референс завершит свой жизненный цикл, а сам владелец данных еще живет, то значение не будет стерто;
- Если переменная, которая является владельцем значения завершит свой жизненный цикл (сотрется с памяти), то референс станет недействительным.

## Разница между передачей владения и референсом
...
Для решения такой проблемы и были придуманы референсы. Вот что будет если мы создадим референс на владельца значения и *позаимствуем данные*:
...
Референс как уже было оговорено выше содержит указатель на переменную, который существует покуда существует переменная (владелец). С помощью передачи референса **мы не меняем владельца значения строки**, мы просто даем пользоваться значением. Сам `&s` тоже не владеет значением, он только на него ссылается.
...
Процесс когда мы берем переменную и создаем для нее референс называется **заимствование** (_наконец-то мы пришли к тому, с чего начинали_). Заимствованием в свою очередь оно называется потому что мы берем указатель на переменную, мы не имеем право дропать само значение. Фактически мы одалживаем у владельца значения (переменной) данные на время, тем самым продляя время жизни переменной, если оно еще не истекло.

> **Важно**:
> 
> Если переменная потеряла владение над данными, то мы не сможем создать для нее референсы, так как конечный указатель фактически никуда не ссылается. Rust сообщит вам об этом во время компиляции с помощью ошибки🦀

## Изменяемые референсы

Если мы попытаемся изменить значение, через референс, то мы сразу же получим ошибку:
...
Для того чтобы создать референс, который будет способен изменять данные у владельца (переменной) нам нужно указать (угадайте что) `mut`:
...
У изменяемых референсов есть несколько серьезных ограничений:

1. Мы можем иметь только один изменяемый референс (таким образом мы избегаем проблемы под названием "Data Race", когда два референса могут находиться в асинхронных методах и одновременно записывать данные в значение)
2. Мы не можем одновременно иметь изменяемый и неизменямый референс к переменной. Функции, которые используют неизменяемый референс не ожидают что данные изменятся, поэтому у нас может быть только референс только одного типа.

К слову, мы можем иметь сколько захотим неизменяемых референсов, несколько функций одновременно могут считывать значения, это допустимо, так как по сути они ничего с этим значением сделать не смогут:
...
Мы также можем объявить изменяемый референс, как только неизменяемый референс перестанет использоваться:
...
// Никаких проблем, так как мы использовали s_ref до объявления изменяемого референса
let mut s_mut_ref = &s;
```
...
- Референсы ссылются не на само значение, а на владельца этого значения;
- Референс не имеет право обнулять значение, которое содержит владелец;
- Будет ли очищаться память, в котором лежит значение зависит только от жизненного цикла владельца.

**Специфично для референсов**:

- Неизменяемых референсов может быть сколько угодно;
- Изменяемый референс должен быть всегда один;
- Изменяемые и неизменяемые референсы не могут использоваться в одно и то же время.

# Слайсы (&str)

**Слайсы** - еще один тип референсов, который в основном используется со строками. Слайс обозначается как `&str`. Абсолютно любой слайс является **неизменяемым**, то есть с помощью ссылки на него нельзя изменить данные владельца данных.
...
Нужно обратить внимание, что перед тем как мы указали от какой переменной мы хотим сделать слайс - мы указали амперсанд, что значит что мы будем использовать не самого владельца переменной, а неизменяемый референс на данную переменную. Если бы мы не указали данный амперсанд, то Rust не скомпилировал бы код и вывел бы следующую ошибку:
...
### Референс на созданную структуру - Self
